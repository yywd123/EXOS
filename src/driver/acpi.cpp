...
initialize(Rsdp *rsdp) {
	...
	if(rsdp->revision == 0) {
		Logger::log(Logger::DEBUG, "ACPI 1.0 detected");
		...
		_iter(acpiTableCount) parseTable((TableHeader *)(uintptr_t)((Rsdt *)rsdtOrXsdt)->entries[i]);
	} else {
		Logger::log(Logger::DEBUG, "ACPI 2.0 or higher detected, revision is 0x@", rsdp->revision);
		Xsdt *xsdt = (Xsdt *)rsdp->xsdtAddress;
		rsdtOrXsdt = (void *)xsdt;
		acpiTableCount = (xsdt->header.length - sizeof(TableHeader)) / sizeof(uintptr_t);
		_iter(acpiTableCount) parseTable((TableHeader *)((Xsdt *)rsdtOrXsdt)->entries[i]);
	}
}
