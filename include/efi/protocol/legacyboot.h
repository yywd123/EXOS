...
(EFIAPI *GET_DEVICE_COUNT) (
    ...
    OUT uint64_t           *DeviceCount,
    OUT uint64_t           *MaxCount
    );
...
(EFIAPI *GET_PRIORITY_AND_TABLE) (
    IN  struct _LEGACY_BOOT_INTERFACE   *This,
    IN  BBS_TYPE        *TableType,
    IN OUT  uint64_t       *PrioritySize, // MaxCount * sizeof(uint8_t)
    OUT     uint64_t       *Priority,
    IN OUT  uint64_t       *TableSize,    // MaxCount * sizeof(BBS_TABLE_ENTRY)
...
typedef struct _LEGACY_BOOT_INTERFACE {
    ...
    //
    uint64_t                   BbsVersion;     // Currently 0x0101
